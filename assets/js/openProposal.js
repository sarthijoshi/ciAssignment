$(document).ready(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(data, function (index, value) {
                ...
                sectionData += "<a class='removeSection' href='#' id='" + value['id'] + "'> (remove)</a><br/><br/>";
            });
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data, function (i) {
                    ...
                    sectionContentssss += "<input type='checkbox' class='checkbox1' name='existingSectionContents' rev='" + data[i]['content'] + "' rel='" + data[i]['content_id'] + "' value='" + data[i]['number'] + " " + data[i]['title'] + "'/>" + data[i]['content'] + "<hr size='10'>";
                });
    ...
    $(document).on('click', '.sections', function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                var numberTitleData = data['section'].split(" ");
                $("#existingNewSectionNumber").val(numberTitleData[0] + numberTitleData[1]);
                $("#existingNewSectionTitle").val(numberTitleData[2]);
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data, function (i) {
                    ...
                    newSectionContent += "<input type='checkbox' class='checkbox2' name='newSectionContents' rev='" + data[i]['content'] + "' rel='" + data[i]['content_id'] + "' value='" + data[i]['number'] + " " + data[i]['title'] + "'/>" + data[i]['content'] + "<hr size='10'>";
                });
    ...
    /**
     * to edit privilege of admin proposal
     * @type Number
     */
    var count = 0;
    $(document).on('click', '.editPrivilege', function () {
        ...
        if (count % 2 != 0) {
            ...
            //ajax to update admin proposal privliges
            var proposalStatus = $('#proposalStatus').val();
            var proposalGroup = $('#proposalGroup').val();
            var proposalAssignedStaff = $('#proposalAssignedStaff').val();
            var trackingNumber = $('#trackingNumber').val();
            var proposalCommittee = $('#proposalCommittee').val();
            $.ajax({
                url: baseUrl + 'index.php/users/updateAdminPrivileges',
                datatype: "applicaton/json",
                type: "POST",
                data: {
                    'proposalStatus': proposalStatus,
                    'proposalGroup': proposalGroup,
                    'proposalAssignedStaff': proposalAssignedStaff,
    ...
    /**
     * Enable proposal name for open proposal
     */
    $(document).on('click', '.nameProposal', function () {
        $(this).text(function (i, text) {
            return text === "edit" ? "save" : "edit";
        });
        $("#proposalNameInput").attr('readonly', false);
    });

    /**
     * Enable proposal name for open proposal
     */
    $(document).on('click', '.markForDeletion', function () {
        var checkBoxVal = $('.markForDeletion').is(':checked');
        if (checkBoxVal == 'true') {
            tinymce.activeEditor.getBody().setAttribute('contenteditable', false);
        } else {
            tinymce.activeEditor.getBody().setAttribute('contenteditable', true);
        }
        $("#instructionLine").attr('readonly', true);
        $("#existingNewSectionNumber").attr('readonly', true);
        $("#existingNewSectionTitle").attr('readonly', true);
    });
    /**
     * Enable proposal name for open proposal
     */
    $(document).on('click', '.removeSection', function () {
       var sectionid = $(this).attr('id');
         $.ajax({
            url: baseUrl + 'index.php/users/removeSection',
            datatype: "applicaton/JSON",
            type: "POST",
            data: {
                'sectionid': sectionid
